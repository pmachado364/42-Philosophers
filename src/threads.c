...
/*   Updated: 2025/04/01 00:31:07 by pmachado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool check_philos(t_table *table)
{
	...
	// ✅ Sanity check (add this at the top of the function)
	if (!table || !table->bigbrains)
	{
		printf("🛑 NULL table or bigbrains\n");
		return false;
	}
	...
	while (1)
	{
		...
		while (++i < table->nbr_thinkers)
		{
			// ✅ Safety check for index (add inside the loop, before accessing i)
			if (i < 0 || i >= table->nbr_thinkers)
			{
				printf("🚨 Invalid index access: i = %d\n", i);
				exit(1);
			}
			// ✅ Safe pointer reference
			t_bigbrain *philo = &table->bigbrains[i];
			...
			if (!table->someone_died && has_philo_died(table, i))
				return (end_simulation(table),
					log_philo_status(table, philo->id, "died 💀"), true);
		}
		usleep(500);
	}
	return (true);
